...
def eiras(filename1,filename2):
    ...
    def importance(model,X_train,label):
        try:
            shap_values = shap.TreeExplainer(model).shap_values(X_train)
            f = plt.figure()
            f=shap.summary_plot(shap_values, X_train, plot_type="bar",label=label)
            return(f)
        except Exception:
            return None
    ...
    for m in models:
        ...
        f=importance(model,X_train,label)
    ...
    #plt.show() 
    plt.savefig('the_roc.jpg', bbox_inches='tight',dpi=600)
    plt.close()  
    return Sensitivity,Specificity,Accuracy,AUC,CI_upper,CI_lower,Youden,Optimal,f




if __name__ == '__main__':
    filepath = ["C:/Users/Meripet/Desktop/JI/test(convert_alive_7d).csv","C:/Users/Meripet/Desktop/JI/result.csv"]





    name=(["LR","SVM","GBDT","XGBoost"])
    Sensitivity,Specificity,Accuracy,AUC,CI_upper,CI_lower,Youden,Optimal,f=eiras(filepath[0],filepath[1])
    dataoutput={'机器学习算法':name,'敏感率': Sensitivity,'特异率':Specificity,'准确率':Accuracy,'曲线下面积':AUC,'CIupper':CI_upper,'CIlower':CI_lower,
    '约旦指数':Youden,'区分阈值':Optimal}
    df= pd.DataFrame(dataoutput)
    df.to_csv(r"C:\Users\Meripet\Desktop\JI\tableszyy.csv",encoding="GBK")
